...
export function UsersEdit({ UserId, onUpdate }) {
  ...
  const usersQuery = useQuery({
    ...
    enabled: open && !!UserId,
    queryFn: async () => {
      const res = await ApiCustomer.get(`/api/user/${UserId}`)
  ...
  const updateMutation = useMutation({
    mutationFn: async (values) => {
      ...
      await ApiCustomer.patch(`/api/user/${UserId}`, payload, {
         headers: {
  ...
  return (
    <>
      ...
      >
        ...
        {usersQuery.isFetching ? <p className="text-sm text-muted-foreground mt-2">Loading user information...</p> : null}
      </FormDialog>
    </>
  )
}
